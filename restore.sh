...
part_list_dialog() {
  ...
  for f in $(ls -l /dev | grep -v ^d | grep -oP 'md[0-9]*'); do echo -e "/dev/$f $(lsblk -d -n -o size /dev/$f)\r"; done | grep -vw -e `echo /dev/"${BRroot##*/}"` -e `echo /dev/"${BRswap##*/}"` -e `echo /dev/"${BRhome##*/}"`  -e `echo /dev/"${BRboot##*/}"`
}

disk_list_dialog() {
  for f in /dev/[hs]d[a-z]; do echo -e "$f $(lsblk -d -n -o size $f)\r"; done
  for f in $(ls -l /dev | grep -v ^d | grep -oP 'md[0-9]*'); do echo -e "/dev/$f $(lsblk -d -n -o size /dev/$f)\r"; done
}
...
check_input() {
  ...
  if [ -n "$BRroot" ]; then
    ...
    for i in $(ls -l /dev | grep -v ^d | grep -oP 'md[0-9]*'); do if [[ /dev/$i == ${BRroot} ]] ; then BRrootcheck="true" ; fi; done
  ...
  if [ -n "$BRswap" ]; then
    ...
    for i in /dev/$(ls -l /dev | grep -v ^d | grep -oP 'md[0-9]*'); do if [[ /dev/$i == ${BRswap} ]] ; then BRswapcheck="true" ; fi; done
  ...
  if [ -n "$BRhome" ]; then
    ...
    for i in /dev/$(ls -l /dev | grep -v ^d | grep -oP 'md[0-9]*'); do if [[ /dev/$i == ${BRhome} ]] ; then BRhomecheck="true" ; fi; done
  ...
  if [ -n "$BRboot" ]; then
    ...
    for i in $(ls -l /dev | grep -v ^d | grep -oP 'md[0-9]*'); do if [[ /dev/$i == ${BRboot} ]] ; then BRbootcheck="true" ; fi; done
  ...
  if [ -n "$BRgrub" ]; then
    ...
    for i in $(ls -l /dev | grep -v ^d | grep -oP 'md[0-9]*'); do if [[ /dev/$i == ${BRgrub} ]] ; then BRgrubcheck="true" ; fi; done
  ...
  if [ -n "$BRsyslinux" ]; then
    ...
    for i in $(ls -l /dev | grep -v ^d | grep -oP 'md[0-9]*'); do if [[ /dev/$i == ${BRsyslinux} ]] ; then BRsyslinuxcheck="true" ; fi; done
...
if [ $BRinterface = "CLI" ]; then
  ...
  bootloader_list=(`for f in /dev/[hs]d[a-z]; do echo -e "$f";  done
                    for f in $(ls -l /dev | grep -v ^d | grep -oP 'md[0-9]*'); do echo -e "/dev/$f";  done`)
  editorlist=(nano vi)
  update_list
