...
set_args() {
  ...
  if [ "$BR_TAB" = "0" ]; then
    ...
    if [ ! "$BC_ENCRYPTION" = "none" ]; then
      SCR_ARGS+=(-E "$BC_ENCRYPTION")
    fi

    if [ -n "$BC_PASSPHRASE" ] && [ ! "$BC_ENCRYPTION" = "none" ]; then
      SCR_ARGS+=(-P "$BC_PASSPHRASE")
    fi
...
lost+found">
                        ...
                        <vbox>
                                <text use-markup="true" label="<big>System Tar &amp; Restore</big>"></text>
                                <text label="Version 7.0 tritonas00@gmail.com 2012-2018"></text>
                                <text use-markup="true" label="<small>Backup and Restore your system using tar or Transfer it with rsync</small>"></text>
                                <hseparator></hseparator>
                                <vbox scrollable="true" shadow-type="0">
